...
#Use MODIS data to extract time series of (I) the average fractional snowcover (FSC), (II) the average NDVI, NDMI and NDSI,
...
#method='snowfraction' in other scripts. The user can specify whether clouds and permanent waterbodies need to be masked. This
#script requires a shapefile of a single polygon, or a multi-polygon as input.

#Note that in this script (03), snowmelt is not calculated based on pixel-level snowmelt data (i.e. 'pixel_gam' method is not
#implemented). This approach is instead implemented for a single polygon in script "01-RGEE_TomVersluijs_MODIS_Pixels_Snowmelt.R"
...
##################################################################################################################################
   ...
   #(6): Automatically define some additional parameters
     ...
     file_conn <- file(paste0(here(), "/Output/MODIS/03_Polygons_Snowmelt/", timestamp, "_", data_ID, "_Parameters.txt"), "w")
...
##################################################################################################################################
    ...
    #(7): Load shapefile (i.e. a single polygon or a multipolygon)
       ...
       ggsave(plot=p1, paste0(here(), "/Output/MODIS/03_Polygons_Snowmelt/", timestamp, "_", data_ID, "_Polygons.pdf"), width=10, height=8)
    ...
    #(10) Create a timeseries gif of RGB and NDSI images for the shapefile area
      
      # #Check number of images in collection
      #  MODIS_col$size()$getInfo()
      
      # #Create a timelapse video of RGB band
      #  videoArgs <- list(dimensions=300, region=aoi_Shapefile$geometry(),framesPerSecond=5, crs='EPSG:3857', bands=c("sur_refl_b01", "sur_refl_b04", "sur_refl_b03"),
      ...
      #  videoArgs <- list(dimensions=200, region=aoi_Shapefile$geometry(), framesPerSecond=5, crs='EPSG:3857', bands=c('vis-red', 'vis-green', 'vis-blue'), min=0, max=255)
  ...
  if(mask_clouds==TRUE){
    ...
    #clouds are filtered if one or both of these two algorithms detects clouds (Wilson et al 2014).
     ...
     #(12): Exclude all images from the image collection that have a CloudFraction value >= max_cloud_fraction, and mask all cloud pixels within the remaining images
       ...
       #  videoArgs <- list(dimensions=280, region=aoi_Shapefile$geometry(),framesPerSecond=5, crs='EPSG:3857', bands=c("sur_refl_b01", "sur_refl_b04", "sur_refl_b03"), min=100, max=10000, gamma=c(1.9, 1.7, 1.7))
       ...
       #  videoArgs <- list(dimensions=380, region=aoi_Shapefile$geometry, framesPerSecond=5, crs='EPSG:3857', bands=c('vis-red', 'vis-green', 'vis-blue'), min=0, max=255)
...
##################################################################################################################################
  ...
  if(mask_water==TRUE){
     ...
     #(C): Apply the water masking function to each image in the collection:
       ...
       # videoArgs <- list(dimensions=380, region=aoi_Shapefile$geometry(),framesPerSecond=5, crs='EPSG:3857', bands=c("sur_refl_b01", "sur_refl_b04", "sur_refl_b03"), min=100, max=10000, gamma=c(1.9, 1.7, 1.7))
...
#VIII: Create timelapse videos of the RGB bands, and the NDSI-, NDVI-, NDMI- and NDMI-bands (Cloud and Water masked)

##################################################################################################################################

  # #(19): Create a timeseries GIF of RGB images
  #    videoArgs <- list(dimensions=280, region=aoi_Shapefile$geometry(),framesPerSecond=5, crs='EPSG:3857', bands=c("sur_refl_b01", "sur_refl_b04", "sur_refl_b03"), min=0, max=8000, gamma=c(1.9, 1.7, 1.7))
  #    tryCatch({browseURL(MODIS_clouds_filtered$getVideoThumbURL(videoArgs))}, error = function(cond){return("Too many pixels. Reduce dimensions.")})
  ...
  #      return(img$visualize(bands='NDSI', min=-1, max=1.5, palette=palette)$
  #               copyProperties(img, img$propertyNames()))}
  #    S2_snow_masked_RGB <- MODIS_clouds_filtered$map(visFun_NDSI)
  # 
  #    #Create a timelapse video of NDSI band
  #    videoArgs <- list(dimensions=280, region=aoi_Shapefile$geometry(), framesPerSecond=5, crs='EPSG:3857', bands=c('vis-red', 'vis-green', 'vis-blue'), min=0, max=255)
  ...
  #               copyProperties(img, img$propertyNames()))}
  #    S2_ndvi_masked_RGB <- MODIS_clouds_filtered$map(visFun_NDVI)
  # 
  #    #Create a timelapse video of NDSI band
  #    videoArgs <- list(dimensions=280, region=aoi_Shapefile$geometry(), framesPerSecond=5, crs='EPSG:3857', bands=c('vis-red', 'vis-green', 'vis-blue'), min=0, max=255)
  ...
  #    S2_ndmi_masked_RGB <- MODIS_clouds_filtered$map(visFun_NDMI)
  # 
  ...
  #    S2_ndwi_masked_RGB <- MODIS_clouds_filtered$map(visFun_NDWI)
  # 
