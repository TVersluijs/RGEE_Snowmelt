...
##################################################################################################################################
      ...
      #(0): Clear workspaceand set python environment
       rm(list=ls())
       library(here)
...
##################################################################################################################################       
 ...
 #(5): Specify parameters used in the analysis
   ...
   #(c) Dates
     ...
     start_date <- paste0(year_ID, "-08-01") #choose date (well) before the first snowfall occurs within the study site
     end_date <- paste0(year_ID, "-12-01") #choose date (well) after last date of tracking
...
##################################################################################################################################
      ...
      #(6) Read study area shapefile as a feature collection and clip image collection to area of interest
   ...
   if(mask_clouds==TRUE){      
     ...
     #clouds are filtered if one or both of these two algorithms detects clouds (Wilson et al 2014).
      ...
      #(10) Add cloud information within the aoi to the image collection: 
        ...
        #Especially during the period before snowfall, MOD35 seems to always detect at least 25% cloudcover, while PGE11 detects many 
        ...
        # #(for debugging) Manually compare MODIS RGB images to the different cloud detection algorithms during the most critical period for snowfall.
        ...
        #  image <- MODIS_col$
        #     filterDate(paste0(year_ID, "-08-19"), end_date)$
        #     first()$
...
#VII: Calculate the date of snowfall for every pixel within the study area by fitting a GAM through the NDSI data
        
##################################################################################################################################            
         
      #(13): Calculate the date of snowfall (NDSI >= NDSI_threshold) for every pixel within the study area (bounding box!)
         ...
         #(B): Transform feature collection to a dataframe:
           ...
           #We use ee_table_to_drive() to prevent memory limits
            ...
            task_vector1 <- ee_table_to_drive(
              ...
              description = paste0(data_ID, "_Pixel_NDSI_Snowfall"),
              fileFormat = "CSV",
         ...
         #(C): Calculate the date of snowfall for each pixel in the dataframe
           ...
           #determine date when NDSI>NDSI_threshold and store this date of snowfall together with 
           ...
           #Specify function to sequentially remove outliers from the dataset for each pixel (results in better estimate of day of snowfall).
           ...
           #Run the 'f_detect_threshold_date_parallel_2' function over all data subsets, combine the results and save the resulting dataframe and plots
            print("Calculate the date of snowfall for each pixel within aoi:")
            results <- lapply(1:length(pixelIDs_split), FUN=f_detect_threshold_date_parallel_2, 
                              ...
                              x="doy", direction="up", y_threshold=NDSI_threshold)
           ...
           #Store date of snowfall per pixel within aoi (bounding box!) as a dataframe
           ...
           #The dataframe df_pixel_snowfall now contains the date of snowfall for each individual pixel_ID (514584 10mx10m pixels)
           #within the aoi (i.e. the defined bounding box). To be able to plot these data we transform this dataframe to a feature 
           #collection and then transform this feature collection to an image.
            
       #(14): Transform df_pixel_snowfall to a feature collection with random geometry
            
           #Generate some random longitude and latitude values as this is required for an sf object:
            df_pixel_snowfall$lon <- runif(nrow(df_pixel_snowfall), 0, 74)
            df_pixel_snowfall$lat <- runif(nrow(df_pixel_snowfall), -20, 20)
            
           #Prevent NA in doy_snowfall column (earthengine cannot deal with NA)  
            df_pixel_snowfall$doy_snowfall[is.na(df_pixel_snowfall$doy_snowfall)] <- -9999
           ...
           #memory limits prevent us from processing the whole dataframe at once. Therefore we split it up into subsets with 'chunk_size' rows
            chunk_size = 2000
            rowIDs <- 1:nrow(df_pixel_snowfall)
           ...
           #Iterate through all dataframe subsets, convert each to a feature collection, and append each to the feature collection FC_initial.
            ...
            for(i in 1:length(rowIDs_split)){ 
              ...
              #Select subset of dataframe:
               df_tmp <- df_pixel_snowfall[rowID_min:rowID_max,]
              
              #Change subset-dataframe to a SF object 
               print("Transform df_pixel_snowfall to a feature collection with random geometry:")
              ...
              #Change sf object to an earth engine feature collection by uploading it to the asset folder
               FC_tmp <- sf_as_ee(
                 x = df_sf_tmp,
                 assetId = paste0(path_asset, "/", data_ID, "_df_pixel_snowfall_", i),
                 overwrite = FALSE,
                 monitoring = TRUE,
            ...
            FC_pixels_snowfall <- ee$FeatureCollection(FC_initial)
            FC_pixels_snowfall$first()$getInfo()
            FC_pixels_snowfall$size()$getInfo()
            
            #Inspect assets folder:
             ee_manage_quota()
             ee_manage_assetlist(path_asset)
            
        #(15): Add geometry (latitude and longitude) of each pixel_ID to FC_pixels_snowfall
             
           #The feature collection that we want to construct should contain 'pixel_ID' and 'doy_snowfall' as properties and should contain
           #the original geometry corresponding to each pixel_ID. So far, FC_pixels_snowfall contains a separate feature for each pixel, where
           #each feature contains doy_snowfall and pixel_ID as a property. However, the geometry (lan/lon) of each feature is randomly chosen. 
           ...
           #The final step is then to join FC_pixels_snowfall to FC_pixels_distinct based on an inner join with pixel_ID.
           ...
           #we need to split up our study area in several sub-areas (i.e. create separate adjacent shapefiles using QGIS).
            ...
            #(F): Reduce feature collection properties to only contain pixel_ID  
             ...
             #and contains the original lat/lon geometry of every pixel. We will now add doy_snowfall as a new property
             #to FC_pixels_distinct by merging it with FC_pixels_snowfall.
             
            #(G): Add doy_snowfall as property to FC_pixels_distinct   
              ...
              #Apply the join.
               Pixel_ID_Join <- innerJoin$apply(FC_pixels_distinct, FC_pixels_snowfall,  Filter_pixel_ID)
            ...
            #(H): To speed-up further computations with this feature collection we as an intermediate step upload and re-download it
              ...
              #Delete FC_pixels_snowfall_optimized if it already occured in the asset folder:
               tryCatch({ee_manage_delete(paste0(path_asset, "/", data_ID, "_FC_pixels_snowfall_optimized"))}, 
                        error = function(cond){return("Path did not yet exist - no folder deleted")})
               
              #Upload to asset folder:
               assetid2 <- paste0(path_asset, "/", data_ID, "_FC_pixels_snowfall_optimized")
              ...
              #Get feature collection from asset folder and create FC_pixels_snowfall_optimized
               #assetid2=paste0("users/escape/", data_ID, "/", data_ID, "_FC_pixels_snowfall_optimized")
               FC_pixels_snowfall_optimized <- ee$FeatureCollection(assetid2) 
               #FC_pixels_snowfall_optimized$first()$getInfo()
               #FC_pixels_snowfall_optimized$size()$getInfo()
           
        #(16): Transform the Feature collection FC_pixels_snowfall_optimized to an image (with doy_snowfall as an image band)
               
              #(A): Reduce feature collection FC_pixels_snowfall_optimized to an Image with a 500m resolution:    
               image_snowfall <- FC_pixels_snowfall_optimized$
                 filter(ee$Filter$notNull(list('doy_snowfall')))$ #pre-filter data for nulls that cannot be turned into an image
                 filter(ee$Filter$neq(name='doy_snowfall', value=-9999))$ #pre-filter data for -9999 values
                 reduceToImage(properties=list('doy_snowfall'), reducer=ee$Reducer$first()$setOutputs(list('doy_snowfall')))$
               ...
               #image_snowfall$projection()$getInfo()
               
              #(B): Clip image along The specified shapefile.
               image_snowfall <- image_snowfall$clipToCollection(aoi_Shapefile)
               
              #(C): Extract day of snowfall in year of interest for a single point
               ee_extract(x=image_snowfall, y=coordinates_point, fun=ee$Reducer$first(), scale=resolution, sf=TRUE)
             
              #(D): Plot snowfall day of year as a coloured image
               MODIS_image <- MODIS_clouds_filtered$filterDate(paste0(year_ID, "-09-10"), end_date)$first()#$reproject(crs=crs, crsTransform=NULL, scale=resolution)
               Map$setCenter(coordinates_point$getInfo()$coordinates[1], coordinates_point$getInfo()$coordinates[2], 10)
               ...
               Map$addLayer(image_snowfall,list(bands="doy_snowfall", min=start_date_doy, max=end_date_doy, palette=c('green', 'yellow', 'red')), 'Snowfall_doy')
          
              #(E): Export original image to Google Drive (takes c.a. 2 minutes):
             
                #Create task to export the original doy_snowfall image to Google Drive  
                 task_vector3 <- ee_image_to_drive(
                  fileFormat='GeoTIFF',
                  image=image_snowfall,
                  description=paste0(data_ID, '_PixelSnowfallDoy_Image_DoySnowfall'),
                  region=aoi,
              ...
              #(F): Export RGB image to Google Drive (takes c.a. 2 minutes):
                ...
                #Convert original image to an RGB image:
                 image_snowfall_RGB <- image_snowfall$visualize(bands=c('doy_snowfall'), min=start_date_doy, max=end_date_doy, palette=c('green', 'yellow', 'red'))
                 #ee_print(image_snowfall_RGB)
                 #image_snowfall_RGB$projection()$getInfo()

                #Create task to export RGB image to Google Drive:
                 task_vector4 <- ee_image_to_drive(
                   fileFormat='GeoTIFF',
                   image=image_snowfall_RGB,
                   description=paste0(data_ID, '_PixelSnowfallDoy_Image_RGB'),
                   region=aoi,
                   #scale=ee$Number(resolution), #defaults to native resolution of image asset.
                   crs="EPSG:3857", #Coordinate reference system of projection of exported image
                   maxPixels=1e9, #maximum allowed number of pixels in exported image
       ...
       #17: Extract the date of snowfall for all pixels within image_snowfall (i.e. clipped by aoi_Shapefile)         
              ...
              #(A): Extract the date of snowfall at each pixel within image_snowfall and store each pixel value as a separate feature.
              #The resulting output is a feature collection of all features (pixels) for the current image
               FC_snowfall <- image_snowfall$sample( #sampling is done automatically for all Bands of the input image
              ...
              #(B): Make sure there is a doy_snowfall value at each feature within the feature collection:
              #Set the band value to a no data value of -9999 for all features where the band value is NULL.
               FC_snowfall <- FC_snowfall$map(function(feature){
                 doy_snowfall <- ee$List(list(feature$get('doy_snowfall'), -9999))$reduce(ee$Reducer$firstNonNull())
                 return(feature$set("doy_snowfall", doy_snowfall))})
              ...
              #(D): Transform feature collection FC_snowfall to a dataframe:
                 
                #We use ee_table_to_drive() to prevent memory limits
                 a=Sys.time()
                 task_vector5 <- ee_table_to_drive(
                   collection = FC_snowfall,
                   description = paste0(data_ID, "_Pixel_Snowfall_shapefile"),
                   fileFormat = "CSV",
                   selectors = c('doy_snowfall', 'lat', 'lon')
                 )
                ...
                #Make sure each latitude/longitude combination gets its own pixel_ID (takes ca 1 minute):
                 df_pixel_snowfall_shapefile$pixel_ID <- paste0(format(df_pixel_snowfall_shapefile$lat, nsmall = 5), "_", format(df_pixel_snowfall_shapefile$lon, nsmall = 5))
                 
                #Only select columns "pixel_ID" and "doy_snowfall"
...
###########################################################################################################################################################################
  ...
  # #OPTIONAL 1: Extract snowfall data from image_snowfall at points of interest
  # 
  #   #(0): Clear workspace
  #    rm(list=ls())
  # 
  ...
  #       start_date <- paste0(year_ID, "-03-15") #choose date (well) before the first snowfall occurs within the study site
  ...
  #      #modisProjection$getInfo()
  # 
  #   #(9): Re-construct generated snowfall image ('image_snowfall')
  # 
  #      #Load feature collection FC_pixels_snowfall_optimized
  #      assetid2=paste0(path_asset, "/", data_ID, "_FC_pixels_snowfall_optimized")
  #      FC_pixels_snowfall_optimized <- ee$FeatureCollection(assetid2)
  # 
  #      #Reduce feature collection FC_pixels_snowfall_optimized to an Image with MODIS sinusoidal projection
  #       image_snowfall <- FC_pixels_snowfall_optimized$
  #         filter(ee$Filter$notNull(list('doy_snowfall')))$ #pre-filter data for nulls that cannot be turned into an image
  #         filter(ee$Filter$neq(name='doy_snowfall', value=-9999))$ #pre-filter data for -9999 values
  #         reduceToImage(properties=list('doy_snowfall'), reducer=ee$Reducer$first()$setOutputs(list('doy_snowfall')))$
  ...
  #        image_snowfall <- image_snowfall$clipToCollection(aoi_Shapefile)
  # 
  #      #Plot snowfall image on a map. Note that 'Map' automatically converts to mercator projection!
  ...
  # 
  #   #(11): Extract Sentinel-2 snowfall_doy values for points of interest from image_snowfall (output a feature collection)
  # 
  #     #Extract snowfall_doy values within the current image at each point in the Locations feature collection. We add this doy
  #     #as a property to each feature (i.e. movement position). The resulting output is a feature collection for the current img
  #     #where the snowfall_doy values are added as a property for each movement position.
  #      FC_image <- image_snowfall$reduceRegions(collection = Locations,
  #                                               reducer = ee$Reducer$mean()$setOutputs(list('Snowfall_doy')),
  #                                               #scale=resolution,
  #                                               crs=modisProjection,
  #                                               crsTransform=NULL)
  # 
  ...
  #     #the boundaries of the image. Therefore we manually have to add an empty snowfall_doy property to those features.
  #     #We therefore set the snowfall_doy value to a no data value of -9999 for all features where the snowfall_doy value is NULL.
  #      FC_image <- FC_image$map(function(feature){
  #          snow <- ee$List(list(feature$get('Snowfall_doy'), -9999))$reduce(ee$Reducer$firstNonNull())
  #          return(feature$set("Snowfall_doy", snow))
  #          })
  # 
  ...
  #       snow_aoi <- unlist(FC_image$aggregate_array('Snowfall_doy')$getInfo())
  #       location_aoi <- unlist(FC_image$aggregate_array('LocationID')$getInfo())
  # 
  #      #Store all data in a dataframe
  #       Locations_BandValues <- data.frame(Snowfall_doy=round(as.numeric(snow_aoi), 5),
  #                                           LocationID=location_aoi)
  # 
  #      #Change -9999 to NA
  #       Locations_BandValues$Snowfall_doy[Locations_BandValues$Snowfall_doy < -9000] <- NA
  # 
  ...
  #        p1 <- ggplot()+
  #          geom_line(data=ZAC_Paths, aes(x=Date_doy, y=Snowfall_doy ), col="#1620de", lwd=1.25)+
  #          geom_abline()+
  #          facet_wrap(~ChickID, ncol=ceiling(length(unique(ZAC_Paths$ChickID))^0.5))+
  #          geom_smooth(data=ZAC_Paths, aes(x=Date_doy, y=Snowfall_doy), method="lm", formula=y~x, col="red", size=1.25, se=T)+
  #          geom_point(data=ZAC_Paths, aes(x=Date_doy, y=Snowfall_doy ), col="black")+
  #          xlab("Day of year") +
  ...
  #        ZAC_Chicks_MeanBandvalue <- ZAC_Paths[!is.na(ZAC_Paths$Snowfall_doy),] %>% group_by(ChickID) %>% summarise(mean_Snowfall_doy=mean(Snowfall_doy),
  ...
  #        #Combine growth parameters with mean Band value during foraging
  #        p2 <- ggplot()+
  #          geom_smooth(data=ZAC_Chicks_MeanBandvalue, aes(x=mean_Snowfall_doy, y=d, size=max_age), method="lm", formula=y~x, col="red", size=1.25)+
  #          geom_point(data=ZAC_Chicks_MeanBandvalue, aes(x=mean_Snowfall_doy, y=d, size=max_age), col="black")+
  #          xlab("Mean day of snowfall across foraging locations") +
  ...
  #        lmm1 <- lme(d ~ mean_Snowfall_doy, random=~1|Brood, data=ZAC_Chicks_MeanBandvalue, method="ML")
  #        lmm2 <- lme(d ~ mean_Snowfall_doy + I(mean_Snowfall_doy^2), random=~1|Brood, data=ZAC_Chicks_MeanBandvalue, method="ML")
  #        MuMIn::AICc(lmm0, lmm1, lmm2)
  #        rm(lmm0, lmm1, lmm2)
  # 
  #        #Calculate mean visiting date vs mean date of snowfall for all chicks (55 days!)
  #        mean(ZAC_Chicks_MeanBandvalue$mean_doy - ZAC_Chicks_MeanBandvalue$mean_Snowfall_doy)
    
#####################################################################################################################################          
       
# #OPTIONAL 2: Add movement trajectories to Snowfall map:
# 
#   #(0): Clear workspace
#    rm(list=ls())
# 
...
#     data_ID <- paste0(data_ID, "_", MODIS_cloud_masking_algorithm)
# 
#   #(4): Load the .tif file for the snowfall image. This .tif file has to be downloaded manually from my google drive
#   #and placed in the local working directory (/Input) of this script.
#    tif=paste0("Input/", data_ID, "_PixelSnowfallDoy_Image_RGB.tif")
#    x=read_stars(tif)
#    x_RGB <- st_rgb(x,3)
# 
#   #(5): Plot snowfall map
...
# 
#     #Plot movement data on top of snowfall map
