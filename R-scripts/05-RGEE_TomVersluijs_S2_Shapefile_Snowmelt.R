  ...
  #(18): Count the total number of unmasked pixels and the total number of pixels per doy within aoi_Shapefile
    ...
    if(pixel_counts==TRUE){
     ...
     #(B): Extract pixel_counts of all images in image collection for aoi_Shapefile
       ...
       #We use ee_table_to_drive() to prevent memory limits
        task_vector0 <- ee_table_to_drive(
          ...
          description = paste0(current_timestamp0, "_", data_ID, "_Res", resolution, "_Data_Pixel_Counts_polygon"),
          fileFormat = "CSV",
       ...
       #Export results to local folder
        exported_stats <- ee_drive_to_local(task = task_vector0, dsn=paste0(here(), "/Output/S2/05_Shapefile_Snowmelt/", timestamp, "_", data_ID, "_Res", resolution, "_Data_Pixel_Counts_polygon"))
     ...
     #(D): Add missing dates with 0 unmasked pixels to the dataframe
      ...
      write.csv(df_pixelcount, file=paste0(here(), "/Output/S2/05_Shapefile_Snowmelt/", timestamp, "_", data_ID, "_Res", resolution, "_Data_Pixel_Counts_polygon.csv"), quote=FALSE, row.names=FALSE)
