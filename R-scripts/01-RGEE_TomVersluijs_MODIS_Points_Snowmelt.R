...
################################################################################################################################################################################
     ...
     if("avg_NDSI" %in% method){
       ...
       #Transform the feature collection with average Band values per day per location to a dataframe by exporting it
         ...
         #(I): Google Drive (preferred option, slightly faster than using Asset folder)
           ...
           #Setup task
            task_vector <- ee_table_to_drive(
             ...
             description = paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_Data_MeanBandvalues"),
             folder="RGEE_tmp",
             fileFormat="CSV",
           ...
           #Import results
            exported_stats <- ee_drive_to_local(task=task_vector, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_Data_MeanBandvalues"))
         ...
         #                                           )
           ...
           #  write.csv(df_Locations_Bandvalues_new, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_Data_MeanBandvalues_polygon.csv"), row.names = FALSE)
         ...
         #(H.2): Extract fraction of snowcover within the buffer zone for each image in the image collection
           ...
           #We export the data instead of using aggregate_array() as the latter might fail due to computation timeouts.
             ...
             #Setup task
              task_vector <- ee_table_to_drive(
                collection= FC_merged,
                description = paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_NDSI", NDSI_threshold_char, "_Data_SnowFraction"),
                folder="RGEE_tmp",
                fileFormat="CSV",
             ...
             #Import results
              exported_stats <- ee_drive_to_local(task=task_vector, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_NDSI", NDSI_threshold_char, "_Data_SnowFraction"))
             ...
             # #Save dataframe for current location
             #  write.csv(df_Locations_SnowFraction_new, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_", Location_ID, "_Data_SnowFraction_polygon.csv"), row.names = FALSE)
           }
        ...
        #(I.2): Transform feature collection to a dataframe:
          ...
          #We use ee_table_to_drive() to prevent memory limits
           ...
           task_vector1 <- ee_table_to_drive(
             collection = FC_merged,
             description = paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_Data_Pixel_NDSI_bbox"),
             fileFormat = "CSV",
           ...
           exported_stats <- ee_drive_to_local(task = task_vector1, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_Data_Pixel_NDSI_bbox"))
          ...
          #  df_pixel_ndsi <- read.csv(paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_Data_Pixel_NDSI_bbox.csv"))
            ...
            #The final step is then to join FC_pixels_snowmelt to FC_pixels_distinct based on an inner join with pixel_ID.
             ...
             #(H): To speed-up further computations with this feature collection we as an intermediate step upload and re-download it
               ...
               #Delete FC_pixels_snowmelt_optimized if it already occured in the asset folder:
                tryCatch({ee_manage_delete(paste0(path_asset, "/", timestamp, "_", data_ID, "_Location_", Location_i, "_FC_pixels_snowmelt_optimized"))},
         ...
         for(NDSI_threshold in NDSI_threshold_vector){  
                ...
                if(export_pixel_image_to_drive==TRUE){
                   ...
                   #Create task to export the original doy_snowmelt image to Google Drive
                    task_vector3 <- ee_image_to_drive(
                     ...
                     description=paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, '_Pixel_Image_DoySnowmelt'),
                     region=aoi,
                   ...
                   #Start and monitor export task:
                    ...
                    ee_drive_to_local(task = task_vector3, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, '_Pixel_Image_DoySnowmelt'))
                 ...
                 #(F): Export RGB image to Google Drive (takes c.a. 2 minutes):
                   ...
                   #Create task to export RGB image to Google Drive:
                    task_vector4 <- ee_image_to_drive(
                      ...
                      description=paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, '_Pixel_Image_DoySnowmelt_RGB'),
                      region=aoi,
                      #scale=ee$Number(resolution), #defaults to native resolution of image asset.
                      crs="EPSG:3857", #Coordinate reference system of projection of exported image
                      maxPixels=1e9, #maximum allowed number of pixels in exported image
                   ...
                   #Start and monitor export task:
                    ...
                    ee_drive_to_local(task = task_vector4, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, '_Pixel_Image_DoySnowmelt_RGB'))
               ...
               #(D): Transform feature collection FC_snowmelt to a dataframe:
                 ...
                 #We use ee_table_to_drive() to prevent memory limits
                  ...
                  task_vector5 <- ee_table_to_drive(
                    ...
                    description = paste0(timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, "_Data_Pixel_Snowmelt_shapefile"),
                    fileFormat = "CSV",
                 ...
                 #Execute task
                  ...
                  exported_stats <- ee_drive_to_local(task = task_vector5, dsn=paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, "_Data_Pixel_Snowmelt_shapefile"))
           ...
           #(I.8): Calculate the fraction of snowcovered pixels within aoi_Shapefile per doy
               ...
               #(A) Create dataframe df_Locations_Pixel_SnowFraction_new  
                 ...
                 # #Save dataframe for current location
                 # write.csv(df_Locations_Pixel_SnowFraction_new, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/DataLocations/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Location_", Location_i, "_NDSI", NDSI_threshold_char, "_Data_Pixel_SnowFraction_polygon.csv"), row.names = FALSE)
 ...
 #Save combined dataframe for all locations
   ...
   if("avg_NDSI" %in% method){ 
    write.csv(df_Locations_BandValues, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_MeanBandvalues_polygon.csv"), row.names = FALSE)
    #df_Locations_BandValues <- read.csv(paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_MeanBandvalues_polygon.csv"), header = T)
    }
    
   #Store dataframe when method="snowfraction"
   if("snowfraction" %in% method){ 
    write.csv(df_Locations_SnowFraction, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_SnowFraction_polygon.csv"), row.names = FALSE)
    #df_Locations_SnowFraction <- read.csv(paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_SnowFraction_polygon.csv"), header=T)
    }
    
   #Store dataframe when method="pixel_gam"
   if("pixel_gam" %in% method){ 
    write.csv(df_Locations_Pixel_SnowFraction, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_Pixel_SnowFraction_polygon.csv"), row.names = FALSE)
    #df_Locations_Pixel_SnowFraction <- read.csv(paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Data_Pixel_SnowFraction_polygon.csv"), header=T)
    }
 ...
 #(I - avg_NDSI) - Fit a Generalized Additive Model (GAM) through the NDSI data for each Location
  ...
  if("avg_NDSI" %in% method){
   ...
   #(D) Plot the raw NDSI datapoints and gam predictions for each Location:
     ...
     #    theme_tom()
     # 
     #  ggsave(plot=p_Locations_NDSI, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Plot_NDSI_polygon.pdf"), width=10, height = 8)
   ...
   #(E) Calculate at which day of year the average NDSI is equal to NDSI_threshold_vector for each location using predictions from mod_gam
     ...
     #Save dates of snowmelt per Location as a .csv file
      write.csv(df_Snowmelt_NDSI_Locations, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Snowmelt_NDSI_polygon.csv"), row.names = FALSE)
  ...
  if("snowfraction" %in% method){
   ...
   #(E) Calculate at which day of year the SnowFraction is equal to Snowfraction_threshold_vector for each location using predictions from mod_gam
     ...
     #Loop through all locations in the df_Locations_SnowFraction_GAM_predictions dataframe and extract doy for all Snowfraction levels:
      ...
      write.csv(df_SnowFraction_Locations, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Snowmelt_Snowfraction_polygon.csv"), row.names = FALSE)
 ...
 #(III - pixel_gam) - Fit a Generalized Additive Model (GAM) through the pixel-level SnowFraction data
  ...
  if("pixel_gam" %in% method){
   ...
   #(D) Plot the raw pixel-level SnowFraction datapoints and gam predictions for each Location:
    ...
    #    theme_tom()
    # 
    #   ggsave(plot=p_Locations_Pixel_SnowFraction, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Plot_Pixel_SnowFraction_polygon.pdf"), width=10, height = 8)
   ...
   #(E) Calculate at which day of year the pixel-level SnowFraction is equal to Snowfraction_threshold_vector for each location using predictions from mod_gam
     ...
     #Save dates of snowmelt per Location per SnowFraction threshold as a .csv file
      write.csv(df_Pixel_SnowFraction_Locations, paste0(here(), "/Output/MODIS/01_Points_Snowmelt/", timestamp, "_", data_ID, "_Buffer", Buffer_radius_m, "_Res", resolution, "_Locations_Snowmelt_Pixel_Snowfraction_polygon.csv"), row.names = FALSE)
