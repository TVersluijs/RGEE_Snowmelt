...
##################################################################################################################################
   ...
   #(5): Automatically define some additional parameters 
     ...
     if(dir.exists(paste0(here(), "/Output/MODIS/Shapefile_Pixel_RelativeBandValue"))==FALSE){dir.create(paste0(here(), "/Output/MODIS/Shapefile_Pixel_RelativeBandValue"), recursive = TRUE)}
...
##################################################################################################################################
    ...
    #(8) Add NDSI and NDVI to the clipped image collection
...
##################################################################################################################################
  ...
  if(mask_clouds==TRUE){
      ...
      #(9) Add cloud information within the aoi to the image collection: 
      ...
      #(10): Exclude all images from the image collection that have a CloudFraction value > max_cloud_fraction, and mask all cloud pixels within the remaining images
        ...
        #snowcovered, the pixels we're interested in might be located in the 10% cloud free area. We sthus only filter
...
#VI: Add mean Band value (NDSI, NDVI, NDMI) of each image as a property to each image in the Sentinel-2 image collection
         
##################################################################################################################################       
         
   #(19): Add mean Band value of each image as a property to each image in the Sentinel-2 image collection
     
     #Add the mean band values within the shapefile area to each separate image by the AddMeanBandValue function over the image collection
      s2_col_composite <- s2_col_composite$
        map(AddMeanBandValue)
     
     # #Check if Mean Band value property has been added to each image (for debugging)
     #  s2_col_composite$first()$propertyNames()$getInfo()
     
   #(20): Substract Sentinel-2 band value at each pixel from the average band value for that image (indicating relative phenology of each pixel at each timestep) 
    
      #Create new bands within each image where the band value (NDSI, NDVI or NDMI) of each pixel is substracted from the average
      #value for that image (NDSI_mean, NDVI_mean or NDMI_mean). This will result in a relative phenology of each pixel compared
      #to the average phenology within that image. Substraction only works between images so we first have to transform the mean
      #band value to a new constant image.
  
      #Map relative band values over the image collection
       s2_col_composite <- s2_col_composite$
         map(get_Relative_NDSI)$
         map(get_Relative_NDVI)$
         map(get_Relative_NDMI)
      
      # #Check if bands have been added (for debugging)
      #  ee_print(s2_col_composite)
        
   #(21): Visualize relative phenology for each band value using timeseries videos
       
      # #To create cleaner videos we filter the image collection to only contain images with < 20% cloud cover
      #  s2_col_composite <- s2_col_composite$
      #     filter(ee$Filter$lt('CloudFraction', 0.2))
       
      # #Check if any image remain where mean band values equal -9999:
      #  any(round(unlist(s2_col_composite$aggregate_array('NDSI_mean')$getInfo()), 3)==-9999)
      #  any(round(unlist(s2_col_composite$aggregate_array('NDVI_mean')$getInfo()), 3)==-9999)
      #  any(round(unlist(s2_col_composite$aggregate_array('NDMI_mean')$getInfo()), 3)==-9999)
      #  #All checks should be FALSE
       
      #Create videos to map relative phenology of NDSI, NDVI and NDMI over time:
       
        #Relative NDSI
         videoArgs <- list(dimensions=400, region=aoi, framesPerSecond=5, crs='EPSG:3857', bands=c('NDSI_Relative'), min=-0.5, max=0.5, palette=c("green", "yellow", "red"))
         browseURL(s2_col_composite$getVideoThumbURL(videoArgs)) 
       
        #Relative NDVI
         videoArgs <- list(dimensions=400, region=aoi, framesPerSecond=5, crs='EPSG:3857', bands=c('NDVI_Relative'), min=-0.5, max=0.5, palette=c("red", "yellow", "green"))
         browseURL(s2_col_composite$getVideoThumbURL(videoArgs)) 
       
        #Relative NDMI
         videoArgs <- list(dimensions=400, region=aoi, framesPerSecond=5, crs='EPSG:3857', bands=c('NDMI_Relative'), min=-0.5, max=0.5, palette=c("green", "yellow", "red"))
         browseURL(s2_col_composite$getVideoThumbURL(videoArgs))  
