...
##################################################################################################################################       
 ...
 #(4): Manually specify parameters of interest
   ...
   #(b) Area of interest
     ...
     shapefile_subarea2 <- "TAY_SubArea2_EPSG4326.shp"
     shapefile_subarea3 <- "TAY_SubArea3_EPSG4326.shp"
     shapefile_subarea4 <- "TAY_SubArea4_EPSG4326.shp"
     ...
     #(9) Create a timeseries gif of RGB images for the shapefile area
        if(gif_output==TRUE){
          ...
          f_img_col_to_gif(img_col=s2_col,  #image collection
                           ...
                           gif_text_col="#FFFFFF",
                           output_fldr=paste0(root_fldr, "/Output/S2/09_Pixels_Snowmelt_LargeAreas/"),
                           file_name="GIF_RGB")
...
##################################################################################################################################
  ...
  if(mask_clouds==FALSE){
      ...
      #Set s2_clouds_filtered equal to s2_col
      s2_clouds_filtered <- s2_col
...
#VII: Create timelapse videos of the RGB bands, and the NDSI-, NDVI-, NDMI- and NDMI-bands for aoi_Polygons (Cloud and Water masked)

##################################################################################################################################

   #Create timeseries GIF animations
   if(gif_output==TRUE){
    
      #(a): GIF of RGB-images
      f_img_col_to_gif(img_col=s2_clouds_filtered,  #image collection
                       RGB_bands=c("B4", "B3", "B2"), #names of RGB-bands in image collection
                       shapefile=aoi_Shapefile, #shapefile of area of interest
                       centroid_buffer_m=0, #buffer zone surrounding centroid of shapefile in meters (to enlarge area for GIF)
                       gif_dimensions=gif_max_pixels, #Maximum dimensions of GIF (pixels)
      ...
      #(b): GIF of NDSI-images
        ...
        s2_clouds_filtered_ndsi <- s2_clouds_filtered$map(ee_utils_pyfunc(function(image){
        ...
        f_img_col_to_gif(img_col=s2_clouds_filtered_ndsi,  #image collection
      ...
      #    s2_clouds_filtered_ndvi <- s2_clouds_filtered$map(ee_utils_pyfunc(function(image){
      ...
      #    f_img_col_to_gif(img_col=s2_clouds_filtered_ndvi,  #image collection
      #                     RGB_bands=c("vis-red", "vis-green", "vis-blue"), #names of RGB-bands in image collection
      #                     shapefile=aoi_Shapefile, #shapefile of area of interest
      #                     centroid_buffer_m=0, #buffer zone surrounding centroid of shapefile in meters (to enlarge area for GIF)
      #                     gif_dimensions=gif_max_pixels, #Maximum dimensions of GIF (pixels)
      ...
      #    s2_clouds_filtered_ndmi <- s2_clouds_filtered$map(ee_utils_pyfunc(function(image){
      ...
      #    f_img_col_to_gif(img_col=s2_clouds_filtered_ndmi,  #image collection
      #                     RGB_bands=c("vis-red", "vis-green", "vis-blue"), #names of RGB-bands in image collection
      #                     shapefile=aoi_Shapefile, #shapefile of area of interest
      #                     centroid_buffer_m=0, #buffer zone surrounding centroid of shapefile in meters (to enlarge area for GIF)
      #                     gif_dimensions=gif_max_pixels, #Maximum dimensions of GIF (pixels)
      ...
      #    s2_clouds_filtered_ndwi <- s2_clouds_filtered$map(ee_utils_pyfunc(function(image){
      ...
      #    f_img_col_to_gif(img_col=s2_clouds_filtered_ndwi,  #image collection
      #                     RGB_bands=c("vis-red", "vis-green", "vis-blue"), #names of RGB-bands in image collection
      #                     shapefile=aoi_Shapefile, #shapefile of area of interest
      #                     centroid_buffer_m=0, #buffer zone surrounding centroid of shapefile in meters (to enlarge area for GIF)
      #                     gif_dimensions=gif_max_pixels, #Maximum dimensions of GIF (pixels)
...
#VIII: Calculate the date of snowmelt for every 10mx10m pixel within the study area by fitting a GAM through the NDSI data
...
##################################################################################################################################            
       ...
       #(20): Calculate the date of snowmelt (NDSI <= NDSI_threshold) for every pixel within the study area 
         ...
         #(C): Calculate the date of snowmelt for each pixel in the dataframe
           ...
           #several sub-datasets (e.g. based on subsets of 5000 unique pixel_IDs).
           ...
           #Split dataset into chuncks of 5,000 pixels
            chunk_size = 5000
       ...
       #(21): Transform df_pixel_snowmelt to a feature collection with random geometry
        ...
        #(22): Add geometry (latitude and longitude) of each pixel_ID to FC_pixels_snowmelt
           ...
           #we need to split up our study area in several sub-areas (i.e. create separate adjacent shapefiles using QGIS), run the code below
           #for each shapefile, and combine the resulting feature collections for each subarea into a single feature collection FC_pixels_distinct.
        
            #(A): Select a single image from the image collection
             img <- s2_clouds_filtered$first()$select("NDSI")
             
             #Read shapefiles per subarea
             aoi_Subarea_1 <- st_read(paste0(root_fldr, "/Input/Shapefiles/", shapefile_subarea1), quiet=T)
             aoi_Subarea_1 <- st_transform(aoi_Subarea_1, crs="EPSG:4326")
             aoi_Subarea_1 <- sf_as_ee(aoi_Subarea_1)
             aoi_Subarea_1 <- ee$FeatureCollection(aoi_Subarea_1)
             
             aoi_Subarea_2 <- st_read(paste0(root_fldr, "/Input/Shapefiles/", shapefile_subarea2), quiet=T)
             aoi_Subarea_2 <- st_transform(aoi_Subarea_2, crs="EPSG:4326")
             aoi_Subarea_2 <- sf_as_ee(aoi_Subarea_2)
             aoi_Subarea_2 <- ee$FeatureCollection(aoi_Subarea_2)
             
             aoi_Subarea_3 <- st_read(paste0(root_fldr, "/Input/Shapefiles/", shapefile_subarea3), quiet=T)
             aoi_Subarea_3 <- st_transform(aoi_Subarea_3, crs="EPSG:4326")
             aoi_Subarea_3 <- sf_as_ee(aoi_Subarea_3)
             aoi_Subarea_3 <- ee$FeatureCollection(aoi_Subarea_3)
             
             aoi_Subarea_4 <- st_read(paste0(root_fldr, "/Input/Shapefiles/", shapefile_subarea4), quiet=T)
             aoi_Subarea_4 <- st_transform(aoi_Subarea_4, crs="EPSG:4326")
             aoi_Subarea_4 <- sf_as_ee(aoi_Subarea_4)
             aoi_Subarea_4 <- ee$FeatureCollection(aoi_Subarea_4)
             
            #(B): Create a feature collection of points at the center of each pixel within each shapefile (each point thus corresponds to a pixel in the subarea image)
             FC_pixels_distinct_1 <- img$sample(
             ...
             FC_pixels_distinct_2 <- img$sample(
             ...
             FC_pixels_distinct_3 <- img$sample(
             ...
             FC_pixels_distinct_4 <- img$sample(
             ...
             #Check that the sum of the pixels in these datasubsets equals the number of rows in df_pixel_snowmelt.
             #This is not necessarily the case anymore due to masking of waterbodies.
             nrow(df_pixel_snowmelt) == FC_pixels_distinct_1$size()$getInfo() + FC_pixels_distinct_2$size()$getInfo() + 
               FC_pixels_distinct_3$size()$getInfo() + FC_pixels_distinct_4$size()$getInfo()
            ...
            #Set the band value to a no data value of -9999 for all features where the band value is NULL.
             FC_pixels_distinct_1 <- FC_pixels_distinct_1$map(function(feature){
              ndsi <- ee$List(list(feature$get('NDSI'), -9999))$reduce(ee$Reducer$firstNonNull())
              return(feature$set("NDSI", ndsi))})
             
             FC_pixels_distinct_2 <- FC_pixels_distinct_2$map(function(feature){
               ndsi <- ee$List(list(feature$get('NDSI'), -9999))$reduce(ee$Reducer$firstNonNull())
               return(feature$set("NDSI", ndsi))})  
             
             FC_pixels_distinct_3 <- FC_pixels_distinct_3$map(function(feature){
               ndsi <- ee$List(list(feature$get('NDSI'), -9999))$reduce(ee$Reducer$firstNonNull())
               return(feature$set("NDSI", ndsi))})  
             
             FC_pixels_distinct_4 <- FC_pixels_distinct_4$map(function(feature){
               ndsi <- ee$List(list(feature$get('NDSI'), -9999))$reduce(ee$Reducer$firstNonNull())
               return(feature$set("NDSI", ndsi))})  
            ...
            #(F): Reduce feature collection properties to only contain 'pixel_ID'
             ...
             #FC_pixels_distinct_4$first()$getInfo()
             #FC_pixels_distinct_4$size()$getInfo()
             
             #Thus, FC_pixels_distinct is a featurecollection that contains a single distinct feature for every pixel_ID,
             #and contains the original lat/lon geometry of every pixel. For the aoi we now have a separate feature collection
             #per subarea. We first need to combine these subareas into a single feature collection.
             
             #Combine the different feature collections per subarea into a single feature collection:
              FC_pixels_distinct <- FC_pixels_distinct_1
              FC_pixels_distinct <- FC_pixels_distinct$merge(FC_pixels_distinct_2)
              FC_pixels_distinct <- FC_pixels_distinct$merge(FC_pixels_distinct_3)
              FC_pixels_distinct <- FC_pixels_distinct$merge(FC_pixels_distinct_4)
              FC_pixels_distinct <- ee$FeatureCollection(FC_pixels_distinct)   
...
#The End
  ...
  {cat("\n") ; cat("\n")
   ...
   cat("\n")
   print(paste0("-Generated data and plots can be found at ", here(), "/Output/S2/09_Pixels_Snowmelt_LargeAreas/"))
   print("--------------------------------------------------------------------------------------------------------------------------")
   cat("\n")}
