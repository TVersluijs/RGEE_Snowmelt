...
Extract_BandValuesAtSubAreas = function(img, FC_initial) {
    ...
    #image where these means are added as properties for each sub area (i.e. different features).
    ...
    #(D): Add total number of snow-covered pixels within each subregion that are not masked by clouds       
    ...
    #(F): Calculate fraction of pixels that are snow covered based on counts of pixels
       ...
       #Map over the feature collection for this image:
        FC_image <- FC_image$map(function(feature){
          ...
          FractionSnowyPixels <- ee$List(list(FractionSnowyPixels)) 
