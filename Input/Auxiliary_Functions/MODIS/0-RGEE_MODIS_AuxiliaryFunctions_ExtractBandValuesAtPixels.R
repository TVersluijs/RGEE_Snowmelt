####################################################################################################################################
#
#                            MODIS - Extract band values at all pixels within the area of interest
#
####################################################################################################################################

#DESCRIPTION: The function Extract_BandValuesAtPixels() can be used to iterate through all images of a MODIS image collection. 
...
#(I): Function to extract BandValues at all pixels within an image and store these values in an expanding Feature Collection:
 Extract_BandValuesAtPixels = function(img, FC_initial) {
    ...
    #The resulting output is a feature collection of all features (pixels) for the current image
     FC_image <- img$sample( #sampling is done automatically for all Bands of the input image
