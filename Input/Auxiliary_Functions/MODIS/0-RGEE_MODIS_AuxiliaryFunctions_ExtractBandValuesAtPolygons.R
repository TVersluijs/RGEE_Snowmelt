####################################################################################################################################
#
#                              MODIS - Extract band values at Polygons within the area of interest
#
####################################################################################################################################

#DESCRIPTION: The function Extract_BandValuesAtPolygons() can be used to iterate through all images of a MODIS image collection.
#For each image, the value of the band of interest (e.g. NDSI) is extracted for each polygon within the feature collection aoi_Polygons.
#The resulting polygon-specific band values are subsequently stored as feature properties in a feature collection together with
#the datetime of the image. Within this feature collection, each polygon thus has a separate feature with its properties. This
#results in a feature collection for all polygons for the current image. This feature collection is then appended to a list of feature
#collections from previous images in the iteration. Thus, at each iteration we extract band values of interest for all polygons within
...
#In addition (currently not activated as code is commented), based on the binary band "SNOW" in each image the fraction of snow-covered
#pixels is calculated by dividing the total number of snow-covered pixels by the total number of pixels within the polygon. This excludes 
#any pixels that are masked due to e.g. cloud cover. Thus, note that this SNOW band and masking of unwanted pixels should occur before 
#running this function.

#The function Extract_BandValuesAtPolygons() takes two arguments. The first argument is the current element of the image collection
...
#(I): Function to extract BandValues for all Polygons within an image and store these values in an expanding Feature Collection:
Extract_BandValuesAtPolygons = function(img, FC_initial) {

    ##For debugging
    #img <- modis_snow_masked$select("FSC_Gascoin2020", "FSC_Aalstad2020", NDSI", "NDVI", "NDMI", "SNOW")$filterDate(paste0(year_ID, "-06-05"), paste0(year_ID, "-07-31"))$first()

    #(A): Calculate the mean NDSI, NDVI, NDMI and SNOW value for the current image within each Polygon in the aoi_Polygons feature collection.
    #We add these mean values as properties to each feature (i.e. polygon). The resulting output is a feature collection for the current
    #image where these means are added as properties for each polygon (i.e. different features). Note that taking the mean of the binary
    #SNOW band results in a fraction of snow covered pixels for each image. The function reduceRegions() does not include masked pixels (i.e.
    #pixels defined as cloud or water) when calculating these means.
    ...
    #
    # #(C): Add total number of pixels within each Polygon that are not masked by clouds nor water
    #
    ...
    #
    #     #(II): Count number of pixels within each Polygon that are not masked by clouds nor water (i.e. both snow covered pixels and pixels without snow cover):
    #      TotalPixels <- snow$reduceRegions(
    #         reducer = ee$Reducer$count(),
    #         collection = aoi_Polygons,
    #         scale = resolution,
    #         crs=crs,
    #         crsTransform=NULL
    #         )
    #
    #     #(III): Only select 'Cluster_ID' (Polygon) and 'count' (renamed to Totalpixels) columns:
    ...
    #           })
    #
    # #(D): Add total number of snow-covered pixels within each polygon that are not masked by clouds nor water
    #
    ...
    #
    #     #(II): Count number of snow covered pixels within each Polygon
    ...
    #(E): Make sure there is a value at each feature within the feature collection:
      ...
      #reduceRegion does not return any band values when all pixels in the polygon are masked by clouds, or do not fall within
