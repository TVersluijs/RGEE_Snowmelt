####################################################################################################################################
#
#                       MODIS - Function for detecting the moment of crossing of threshold using GAMs
#
#################################################################################################################################### 

#DESCRIPTION: This script contains the function 'f_detect_threshold_date_parallel_1' that calculates for which day of year a 
...
#from above to below the threshold value (for snowmelt, using parameter direction="down") or from below to above the threshold 
#(for snowfall, using parameter direction="up"). To speed up processing of all pixels, the script runs the code in parallel 
#(on multiple cores) for each defined subset of pixels stored in the variable 'pixelIDs_split'. The output of each parallel 
#run is stored in a consecutive position in a list. Within each of these list elements the function returns two  outputs, 
#which are each stored in a separate sublist within the main list. The first sublist contains a dataframe with the date on 
...
f_detect_threshold_date_parallel_2 <- function(subset, df_pixel_y=df_pixel_ndsi, pixel_ID_column="pixel_ID", y="NDSI", x="doy", 
                                               direction="down", y_threshold=NDSI_threshold){
  ...
  results <- foreach(pixel=unique(df_subset[,pixel_ID_column]), 
                     ...
                     .options.snow=list(progress = pb_func)) %dopar% {
                       ...
                       #Detect cutoff points for crossing the threshold from above to below the curve (e.g. snowfall)
                       if(direction=="up"){
                       
                         #Detect cutoff points where curve goes from below y_threshold to above y_threshold
                         newdata$cutoff <- ifelse(newdata$gam_predict <= y_threshold, 1, 0)
                         newdata$dif <- c(0, diff(newdata$cutoff))
                         #the column 'cutoff' indicates whether the gam prediction is below (1) or above (0) the y_threshold
                         #the column 'dif' indicates when there is a change from 1 to 0 (-1) or 0 to 1 (1) in the column cutoff
                         #Thus, those rows where the column 'dif' is equal to -1 indicate moments where the y value changes from below
                         #the threshold to above the threshold. It might be possible that this happens multiple times within a season due to
                         #measurement errors or cloud effects. We therefore need to determine which 'cutoff' most likely corresponds to the 
                         #final moment of threshold crossing (e.g. the actual moment of snowfall)
                       ...
                       if(any(newdata$dif<0)){
                         ...
                         #If direction of crossing was from below to above the threshold (e.g. snowfall):
                         if(direction=="up"){
                           
                           #For the period 30 days after each cutoff point, sum the number of days that have a y value SMALLER than y_threshold. 
                           #If a cutoff represents the final moment the threshold is crossed, then we do not expect any days after this moment 
                           #with y < y_threshold. Thus, the closer this sum is to 0, the more likely this cutoff corresponds to the final moment 
                           #that this threshold is crossed (i.e. the actual moment of snowfall).
                           cutoffs$min <- cutoffs$index -30
                           cutoffs$min[cutoffs$min<1] <- 1
                           cutoffs$max <- cutoffs$index + 29
                           cutoffs$max[cutoffs$max>nrow(newdata)] <- nrow(newdata)
                       ...
                       #Store date at which threshold was crossed (e.g. date of snowmelt or snowfall in case of y=NDSI) for each pixel in dataframe:
                       df_pixel_threshold <- base::data.frame(pixel_ID=pixel, x_threshold=x_threshold)
                       
                       #Store all result in the multiresultclass object  
